...
# bugs search (https://udd.debian.org/bugs/) -- one bulk HTTP request
# per scan, not one per package.
...
# UDD's own bugs search, JSON form. 'done=ign'/'merged=ign' drop closed
# and merged bugs server-side; 'cseverity=1' asks back a field UDD
...
# 'min_severity' drops anything less severe than SEVERITY_ORDER's
# cutoff; a bug with an unrecognized severity is dropped rather than
