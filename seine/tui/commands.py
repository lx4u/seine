...
def _target(app, argv):
    ...
    if verb == "connect":
        ...
        # (up to 1.5s) before tearing down, and a direct call would
        # freeze the whole TUI for that wait.
        def _disconnect():
...
# Qualified test name(s) under the overview's selected node, with
# the same no-run-yet fallback OverviewScreen.update_body() uses.
