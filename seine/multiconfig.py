...
def _cohort_tasks(members, prefix):
    ...
    # via its own _record_group(), narrowed to its own packages. Its
    # 'defaults: vault:' came from 'image' (the first member) alone;
    # fold every other member's own in too, or a package this cohort
    # builds for a member that isn't 'image' never sees its vault seed.
    for member in members:
        member.image._builder = image._builder
        image._builder._vault_defaults.update(member.image._vault_defaults())
...
#
# 'files'/'logs' feed logindex.record() over the same 'group_tasks',
# so it stays consistent with analyze.record() about this group's run.
