...
# failing loudly on timeout instead of falling through to an assert
# that then fails somewhere else.
...
# The style covering one character of a Rich Text -- the first span
# containing the offset is the one that paints it, since spans are
# appended in order and never overlap.
...
# render_root_node(): file list with git-status/lock marks, plus
# the same stats render_overview() shows.
class RootNodeRendering(avocado.Test):
    ...
    def test_files_are_marked_with_git_status_and_lock_icon(self):
        ...
        with open(modified, "w") as f:
            f.write(spec_text)
        # modified.lock.yaml auto-pairs with modified.yaml. Use '{}',
        # not empty -- an empty file parses as None, which merge()
        # can't treat as a spec fragment.
...
# no Context/BuildCmd needed, it only reads plain spec fields.
class ImageNodeRendering(avocado.Test):
    ...
    # One image-wide height budget, not one per sibling group -- a
    # smaller top-level partition must not out-render a much bigger
    # LVM volume just because its own sibling group was smaller.
    ...
    # Box art must fit the width it's given, never the BOX_WIDTH
    # default. Only border/side lines are asserted: 'legend:' is
    # plain prose the pane wraps harmlessly.
    ...
    # The outer container's frame stays one consistent color around
    # inner boxes drawn in their own, not the nested row's color.
...
class LogsSectionRendering(avocado.Test):
    ...
    # A cataloged task with no file on disk yet is dimmed, not a
    # clickable path that opens on a missing file.
...
class IssuesRendering(avocado.Test):
    ...
    # Header and source column stay pinned while the rest scrolls,
    # and anything wider than the pane scrolls horizontally.
    ...
    # A '/issues --rescan' never blocks the TUI. '_rescan_once' is
    # stubbed with a gated stand-in so the modal is observable
    # mid-flight without touching the network or a container.
...
class App(avocado.Test):
    ...
    # Tab into the tree, Down twice (past the group node, tested
    # separately below) then Enter picks a leaf. The right pane must
    # show it, not the overview text.
    ...
    # Tab, Down, Enter on the group node itself should show its own
    # spec-files list, not the overview or generic fallback.
    ...
    # Selection is injected by path, not Down presses: the 'image'
    # branch's position depends on spec dict order, not worth hardcoding.
    ...
    # Boxes must fit the actual right pane, far under render.py's
    # BOX_WIDTH default at standard pilot size.
    ...
    # SpecTree and LogViewer. Only widget *display* state is asserted
    # here; the shown text is _read_log()'s own job, tested below.
    ...
    # Probes via dispatch() itself since it's synchronous. Uses invalid
    # commands on purpose: a real one could switch screens mid-loop and
    # leave dispatch operating on an unmounted screen.
    ...
    # Worker callbacks can land mid-transition, before the screen's
    # widgets are composed; these entry points must skip the repaint
    # instead of raising NoMatches into run_test() teardown.
...
# _read_log() (seine/tui/app.py): split out so the read-failure
# fallback is testable on its own, without touching RichLog's
...
class SettingsScreenIntegration(avocado.Test):
    ...
    async def _open(self, pilot, app):
        ...
        # for it to go live rather than racing them with Enter.
        def prompt_live():
            try:
                return not app.screen.query_one("#prompt").disabled
            except Exception:
...
class CastReplay(avocado.Test):
    ...
    def _cast(self):
        ...
        # Twenty numbered lines (pane fits ~14, so early ones scroll
        # out) plus a far-future event to keep the cast still playing
        # when space pauses it.
...
# render_test_node() appends past runs from the test index below the
# live marks. Run dirs are built by hand (no robot run needed) --
# only the files the pane reads exist.
